...
$result = ($month != 0) ? $sp->getdh_thang($month) : $sp->getdon();
?>
              ...
              <table class="w-full whitespace-no-wrap mb-6">
                <thead>
                  <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
                    ...
                    <th class="px-4 py-3">Hành trình đơn hàng đến cho khách</th>
                  </tr>
                </thead>
                <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                  <?php foreach ($result as $i => $rc) : ?>
                    <tr>
                      <td><?= $i + 1 ?></td>
                      <td><?= $rc['id_dh'] ?></td>
                      <td><?= $rc['ten_sp'] ?></td>
                      ...
                      <td>
                        <?php
                        switch ($rc['hanh_trinh']) {
                          case 0:
                            echo 'Đã nhận đơn';
                            break;
                          case 1:
                            echo 'Đã giao đến kho';
                            break;
                          case 2:
                      ...
                      <?php
                      if ($rc['hanh_trinh'] == 0) {
                        echo '<td><a href="/quatrinh?id=' . $rc['id_dh'] . '">Đã nhập kho</a></td>';
                    } elseif ($rc['hanh_trinh'] == 1) {
                        echo '<td><a href="/quatrinh1?id=' . $rc['id_dh'] . '">Đã giao cho shipper</a></td>';
                    } elseif ($rc['hanh_trinh'] == 2) {
                        echo '<td><a href="/quatrinh2?id=' . $rc['id_dh'] . '">Hoàn thành</a></td>';
                    }else{
                      echo'<td>Đơn hàng đã được hoàn thành</td>';
                    }
